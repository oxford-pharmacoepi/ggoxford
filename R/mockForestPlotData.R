...
mockForestPlotData <- function(seed = 1, numVars = 8) {
  ...
  rows1 <- dplyr::tibble(
    ...
    estimate = round((stats::runif(n = numVars, min = -0.6, max = 0.6)),2),
  ...
  rows1 <- rows1 |> dplyr::mutate(estimate = round(exp(estimate),2))
  result <- rbind(rows1,rows2,rows3)
  result <- result |> dplyr::select(-c("se")) |> dplyr::arrange(.data$variable)
  return(result)
}
